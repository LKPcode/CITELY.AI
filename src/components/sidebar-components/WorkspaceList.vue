...
watch( () => route.params.workspace_id, (workspace_id) => {
    if (workspace_id) {
        ...
        if (workspace) {
            ...
            console.log("Workspace not found")
            selectWorkspace(workspace_store.workspace_list.value[0])
        }
    }
})
...
const selectWorkspace = (workspace: Workspace) => {
    ...
    if (workspace_store.selected_workspace.value?.id == workspace.id) 
        return  
