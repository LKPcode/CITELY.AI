<template>
    <nav class="flex items-center justify-between border-t border-gray-200 p-4 ">
  <div @click="emits('previousPage')"  class="-mt-px flex w-0 flex-1 cursor-pointer">
    <a v-if="page_num > 0"  class="inline-flex items-center border-t-2 border-transparent pr-1 pt-4 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700">
  ...
  <div class="hidden md:-mt-px md:flex">
    <span class="text-xs text-grayest">
  ...
  <div @click="emits('nextPage')" class="-mt-px flex w-0 flex-1 justify-end">
    <a v-if="total_papers > 20*(page_num+1)"  class="inline-flex cursor-pointer items-center border-t-2 border-transparent pl-1 pt-4 text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700">
      Next
...
const { page_num, total_papers } = defineProps<{
    page_num: number,
    total_papers: number
}>()
